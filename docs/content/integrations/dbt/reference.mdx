...
| <PyObject module="dagster_dbt" object="load_assets_from_dbt_project" />                        | Deprecated in favor of <PyObject module="dagster_dbt" object="dbt_assets" decorator />.                                                             |
| <PyObject module="dagster_dbt" object="load_assets_from_dbt_manifest" />                       | Deprecated in favor of <PyObject module="dagster_dbt" object="dbt_assets" decorator />.                                                             |
...
In the Dagster project created by the [`dagster-dbt project scaffold`](/\_apidocs/libraries/dagster-dbt#dagster-dbt-project-scaffold) command line interface, we offer you both ways to load your dbt models, depending on your preference:
...
When developing locally, you can run the following command to generate the manifest at run time when developing locally on your dbt and Dagster project:

```shell
DAGSTER_DBT_PARSE_PROJECT_ON_LOAD=1 dagster dev
```

However, when deploying your Dagster project to production, **we recommend generating the manifest at build time** to avoid the overhead of recompiling your dbt project every time your Dagster code is executed. `DAGSTER_DBT_PARSE_PROJECT_ON_LOAD` should be unset and a `manifest.json` should be precompiled and included in the Python package for your Dagster code.
